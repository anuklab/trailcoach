...
export function adherenceStatus(date = today()) {
  ...
  const scored = past.map(r => {
    ...
    if (r.status === 'missed' || r.status === 'skipped') return 0;
    return null; // 'planned' pero su fecha ya pasó sin marcar -> se trata como perdido más abajo
  });
  ...
  const reduced = db.prepare(`SELECT COUNT(*) c FROM sessions WHERE date >= ? AND date < ? AND origin IN ('ajuste','ia') AND change_note IS NOT NULL`).get(from, date).c;
...
export function monthSummary(date = today(), days = 30) {
  ...
  const acts = db.prepare(`SELECT COUNT(*) n, COALESCE(SUM(distance_m),0) dist, COALESCE(SUM(elevation_gain_m),0) dplus,
    ...
    FROM activities WHERE date >= ? AND date < ?`).get(from, date);
  const planned = db.prepare(`SELECT COUNT(*) n FROM sessions WHERE date >= ? AND date < ? AND type != 'rest'`).get(from, date);
  const done = db.prepare(`SELECT COUNT(*) n FROM sessions WHERE date >= ? AND date < ? AND type != 'rest' AND status IN ('done','partial')`).get(from, date);
  return {
