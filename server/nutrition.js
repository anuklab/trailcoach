...
export function addLog(userId, entry) {
  const r = db.prepare(`INSERT INTO nutrition_logs(user_id, date, session_id, minute_mark, product, carbs_g, sodium_mg, caffeine_mg, feeling, gi_issue, notes)
    VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(
    userId, entry.date, entry.session_id || null, entry.minute_mark ?? null, entry.product,
    entry.carbs_g ?? null, entry.sodium_mg ?? null, entry.caffeine_mg ?? null,
    entry.feeling || null, entry.gi_issue ? 1 : 0, entry.notes || null);
  return db.prepare('SELECT * FROM nutrition_logs WHERE id=? AND user_id=?').get(r.lastInsertRowid, userId);
}

export function listLogs(userId, { from, to } = {}) {
  if (from && to) return db.prepare('SELECT * FROM nutrition_logs WHERE user_id = ? AND date BETWEEN ? AND ? ORDER BY date DESC, minute_mark').all(userId, from, to);
  return db.prepare('SELECT * FROM nutrition_logs WHERE user_id = ? ORDER BY date DESC, minute_mark LIMIT 200').all(userId);
}

export function deleteLog(userId, id) { db.prepare('DELETE FROM nutrition_logs WHERE id=? AND user_id=?').run(id, userId); }
...
export function insights(userId) {
  const rows = db.prepare('SELECT product, feeling, gi_issue FROM nutrition_logs WHERE user_id = ?').all(userId);
  const byProduct = {};
  for (const r of rows) {
